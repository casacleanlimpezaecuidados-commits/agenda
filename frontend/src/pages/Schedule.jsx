...
export default function Schedule() {
  ...
  // Substituir funcionário em lote
  const handleReplaceEmployee = async (e) => {
    e.preventDefault();
    setSaving(true);
    try {
      const payload = {
        old_employee_id: parseInt(replaceData.old_employee_id),
        new_employee_id: parseInt(replaceData.new_employee_id),
      };

      if (replaceData.client_id) {
        const schedulesToUpdate = schedules.filter(s => {
          const clientMatch = s.client_id === parseInt(replaceData.client_id);
          const employeeMatch = s.employee_ids?.includes(parseInt(replaceData.old_employee_id));
        ...
        for (const schedule of schedulesToUpdate) {
          const newEmployeeIds = schedule.employee_ids.map(id => 
            id === parseInt(replaceData.old_employee_id) ? parseInt(replaceData.new_employee_id) : id
          );
          await api.put(`/schedules/${schedule.id}`, { employee_ids: newEmployeeIds });
        }
  ...
  // Substituição rápida (do agendamento)
  ...
  const handleQuickReplace = async (e) => {
    ...
    setSaving(true);
    try {
      const newEmployeeIds = [parseInt(quickReplaceData.new_employee_id)];
      await api.put(`/schedules/${quickReplaceData.schedule_id}`, { 
  ...
  const handleStatusChange = async (e) => {
    ...
    setSaving(true);
    try {
      await api.patch(`/schedules/${statusData.schedule_id}/status`, { 
  ...
  const getStatusColor = (status) => {
    const colors = {
      ...
      em_andamento: '#3B82F6',
      concluido: '#10B981',
      concluido_ressalva: '#F59E0B',
      cancelado_cliente: '#EF4444',
      funcionario_faltou: '#DC2626',
    };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => setShowRecurringModal(true)} className="btn-success flex items-center gap-2">
            <Repeat className="w-4 h-4" /> Agenda Recorrente
          </button>
          <button onClick={() => {
            setSelectedClientAddresses([]);
            setReplaceData({ client_id: '', address: '', old_employee_id: '', new_employee_id: '' });
            setShowReplaceModal(true);
          }} className="bg-warning text-white px-4 py-2.5 rounded-xl font-medium hover:bg-amber-600 active:scale-95 transition-all duration-200 shadow-soft flex items-center gap-2">
            <UserCheck className="w-4 h-4" /> Substituir Func.
          </button>
        </div>
      </div>
      ...
      <div className="card-premium p-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => { setCurrentDate(new Date()); loadSchedules(); }} className="btn-secondary text-sm py-1.5">
              Hoje
            </button>
          ...
          <div className="flex items-center gap-2">
            <button onClick={loadSchedules} disabled={loading} className="btn-secondary flex items-center gap-1.5 text-sm py-1.5" title="Atualizar">
        ...
        <div className="card-premium overflow-hidden">
          ...
          <div className="grid grid-cols-7">
            {getDaysInMonth().map((dateObj, index) => {
              ...
              const isCurrentMonth = !dateObj.isOtherMonth;
              ...
              return (
                <div
                  ...
                  className={`
                    min-h-[100px] p-2 border-b border-r border-gray-50 cursor-pointer
                ...
                >
                  <span className={`
                    ...
                    ${isToday(dateObj.day, dateObj.month, dateObj.year) 
                      ? 'bg-primary-800 text-white w-7 h-7 rounded-full flex items-center justify-center' 
                      : 'text-gray-700'}
                  `}>
                    {dateObj.day}
                  </span>
      ...
      {/* Lista de agendamentos (modo semana/dia) */}
      {(viewMode === 'week' || viewMode === 'day') && (
        <div className="card-premium p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-gray-900">
              {viewMode === 'week' ? 'Agendamentos da Semana' : 
               `Agendamentos do dia ${selectedDate?.day || today.getDate()}`}
            </h3>
            <button onClick={loadSchedules} className="btn-secondary flex items-center gap-1.5 text-sm py-1.5">
          ...
          {loading ? (
            <div className="text-center py-12">
              <div className="w-10 h-10 border-4 border-primary-200 border-t-primary-800 rounded-full animate-spin mx-auto mb-3" />
                ...
                <div key={idx} className="p-4 border border-gray-100 rounded-xl hover:shadow-soft transition-all">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {schedule.date ? new Date(schedule.date + 'T00:00:00').toLocaleDateString('pt-BR') : ''} • {schedule.start_time} - {schedule.end_time}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() => {
                          setStatusData({ schedule_id: schedule.id, status: schedule.status });
                      ...
                      <button
                        onClick={() => openQuickReplace(schedule)}
                        className="p-1.5 hover:bg-amber-50 rounded-lg"
                        title="Substituir funcionário"
                      >
                        <UserCheck className="w-4 h-4 text-gray-400 hover:text-warning" />
                      </button>
                      <button
                        onClick={() => handleDeleteSchedule(schedule.id, schedule.client_name)}
                        className="p-1.5 hover:bg-gray-100 rounded-lg"
                        title="Excluir"
                      >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      <span className="truncate">{schedule.address || 'Sem endereço'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-600">
                      <Users className="w-4 h-4 text-gray-400" />
                      <span>{schedule.employee_names?.join(', ') || 'Não informado'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-600">
                      <Phone className="w-4 h-4 text-gray-400" />
                      <span>{schedule.client_phone || 'Não informado'}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <form onSubmit={handleCreateSchedule} className="space-y-4">
                ...
                {selectedClientAddresses.map((addr, idx) => (
                  <label 
                    ...
                    className={`flex items-start gap-3 p-3 border rounded-xl cursor-pointer hover:bg-gray-50 transition-colors ${
                      formData.address === `${addr.street} - ${addr.neighborhood} - ${addr.city}` 
                  ...
                  >
                    <input
                      type="radio"
                      name="address"
                      checked={formData.address === `${addr.street} - ${addr.neighborhood} - ${addr.city}`}
                    ...
                    <div className="flex-1">
                      ...
                      {addr.daysOfWeek?.length > 0 && (
                        <div className="flex gap-1 mt-1">
                          {addr.daysOfWeek.map((day, i) => (
                            <span key={i} className="text-xs bg-primary-100 text-primary-700 px-1.5 py-0.5 rounded-full">{day}</span>
                          ))}
                        </div>
                      )}
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input type="date" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} className="input-premium" required />
            </div>
            <div>
              <label className="label-premium">Início *</label>
              <input type="time" value={formData.start_time} onChange={(e) => setFormData({ ...formData, start_time: e.target.value })} className="input-premium" required />
            </div>
            <div>
              <label className="label-premium">Fim *</label>
              <input type="time" value={formData.end_time} onChange={(e) => setFormData({ ...formData, end_time: e.target.value })} className="input-premium" required />
            </div>
          </div>

          <div>
            <label className="label-premium">Serviço *</label>
            <select value={formData.service} onChange={(e) => setFormData({ ...formData, service: e.target.value })} className="select-premium" required>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-40 overflow-y-auto">
              {employees.map(emp => (
                <label key={emp.id} className="flex items-center gap-2 p-2 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" checked={formData.employee_ids.includes(emp.id)} onChange={() => toggleEmployee(emp.id)} className="rounded" />
          ...
          <div>
            ...
            <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="textarea-premium" rows={2} />
          </div>

          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <button type="button" onClick={() => { setShowNewModal(false); resetForm(); }} className="btn-secondary">Cancelar</button>
      ...
      <Modal isOpen={showRecurringModal} onClose={() => { setShowRecurringModal(false); resetRecurringForm(); }} title="Agenda Recorrente" size="lg">
        <form onSubmit={handleCreateRecurring} className="space-y-4">
          <div>
            <label className="label-premium">Cliente *</label>
            <select value={recurringData.client_id} onChange={(e) => handleClientSelect(e.target.value, true)} className="select-premium" required>
              <option value="">Selecionar cliente...</option>
              {clients.map(c => (
                <option key={c.id} value={c.id}>{c.name}</option>
              ))}
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {selectedClientAddresses.map((addr, idx) => (
                  <label key={idx} className={`flex items-start gap-3 p-3 border rounded-xl cursor-pointer hover:bg-gray-50 ${recurringData.address === `${addr.street} - ${addr.neighborhood} - ${addr.city}` ? 'border-primary-500 bg-primary-50' : 'border-gray-200'}`}>
                    <input type="radio" name="recurring_address" checked={recurringData.address === `${addr.street} - ${addr.neighborhood} - ${addr.city}`} onChange={() => handleAddressSelect(addr, true)} className="mt-0.5" />
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-900">{addr.street}</p>
                      <p className="text-xs text-gray-500">{addr.neighborhood} - {addr.city}</p>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="label-premium">Serviço *</label>
              <select value={recurringData.service} onChange={(e) => setRecurringData({ ...recurringData, service: e.target.value })} className="select-premium" required>
                {SERVICE_TYPES.map(type => (<option key={type} value={type}>{type}</option>))}
              </select>
            </div>
            <div>
              <label className="label-premium">Frequência *</label>
              <select value={recurringData.frequency} onChange={(e) => setRecurringData({ ...recurringData, frequency: e.target.value })} className="select-premium" required>
          ...
          <div>
            ...
            <div className="grid grid-cols-7 gap-2">
              {WEEKDAYS.map((day) => (
                <label key={day} className="flex items-center justify-center gap-1 p-2 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" checked={recurringData.days_of_week.includes(day)} onChange={() => toggleDayOfWeek(day)} className="rounded" />
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-40 overflow-y-auto">
              {employees.map(emp => (
                <label key={emp.id} className="flex items-center gap-2 p-2 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" checked={recurringData.employee_ids.includes(emp.id)} onChange={() => toggleRecurringEmployee(emp.id)} className="rounded" />
                  <span className="text-sm">{emp.name}</span>
                </label>
              ))}
            </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <button type="button" onClick={() => { setShowRecurringModal(false); resetRecurringForm(); }} className="btn-secondary">Cancelar</button>
            <button type="submit" disabled={saving} className="btn-success">{saving ? 'Gerando...' : 'Gerar Agendamentos'}</button>
          </div>
        </form>
      </Modal>

      {/* ========== MODAL - SUBSTITUIR FUNCIONÁRIO (LOTE) ========== */}
      <Modal isOpen={showReplaceModal} onClose={() => { setShowReplaceModal(false); setSelectedClientAddresses([]); }} title="Substituir Funcionário" size="lg">
        <form onSubmit={handleReplaceEmployee} className="space-y-4">
          <div className="bg-blue-50 p-4 rounded-xl text-sm text-blue-800">
            💡 <strong>Substituição em lote:</strong> Filtre por empresa e endereço para substituir apenas os agendamentos desejados.
          </div>

          <div>
            <label className="label-premium">Empresa/Cliente (opcional)</label>
            <select
              value={replaceData.client_id}
              onChange={(e) => {
                const clientId = e.target.value;
                setReplaceData({ ...replaceData, client_id: clientId, address: '' });
                if (clientId) {
                  const client = clients.find(c => c.id === parseInt(clientId));
          ...
          {selectedClientAddresses.length > 0 && replaceData.client_id && (
            <div>
              <label className="label-premium">Endereço específico (opcional)</label>
              <select value={replaceData.address} onChange={(e) => setReplaceData({ ...replaceData, address: e.target.value })} className="select-premium">
                <option value="">Todos os endereços</option>
                {selectedClientAddresses.map((addr, idx) => (
                  <option key={idx} value={`${addr.street} - ${addr.neighborhood} - ${addr.city}`}>{addr.street}</option>
                ))}
              </select>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="label-premium">Funcionário Atual *</label>
              <select value={replaceData.old_employee_id} onChange={(e) => setReplaceData({ ...replaceData, old_employee_id: e.target.value })} className="select-premium" required>
                <option value="">Selecionar...</option>
                {employees.map(emp => (<option key={emp.id} value={emp.id}>{emp.name}</option>))}
              </select>
            </div>
            <div>
              <label className="label-premium">Novo Funcionário *</label>
              <select value={replaceData.new_employee_id} onChange={(e) => setReplaceData({ ...replaceData, new_employee_id: e.target.value })} className="select-premium" required>
                <option value="">Selecionar...</option>
                {employees.filter(e => e.id !== parseInt(replaceData.old_employee_id)).map(emp => (<option key={emp.id} value={emp.id}>{emp.name}</option>))}
              </select>
            </div>
          </div>

          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <button type="button" onClick={() => { setShowReplaceModal(false); setSelectedClientAddresses([]); }} className="btn-secondary">Cancelar</button>
            <button type="submit" disabled={saving} className="bg-warning text-white px-6 py-2.5 rounded-xl font-medium hover:bg-amber-600 transition-all">{saving ? 'Substituindo...' : 'Substituir em Lote'}</button>
          </div>
        </form>
      </Modal>
        ...
        <form onSubmit={handleQuickReplace} className="space-y-4">
          ...
          <div className="bg-gray-50 p-4 rounded-xl space-y-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-medium text-gray-900">{quickReplaceData.client_name}</span>
            </div>
            
            <div className="flex items-center gap-2 text-sm">
              <Calendar className="w-4 h-4 text-gray-400 flex-shrink-0" />
              <span className="text-gray-700">
                {quickReplaceData.date ? new Date(quickReplaceData.date + 'T00:00:00').toLocaleDateString('pt-BR', {
            ...
            <div className="flex items-start gap-2 text-sm">
              ...
              <span className="text-gray-700">{quickReplaceData.address || 'Endereço não informado'}</span>
            </div>

            <div className="flex items-center gap-2 text-sm border-t border-gray-200 pt-2">
              <Users className="w-4 h-4 text-gray-400 flex-shrink-0" />
              <div>
                <span className="text-xs text-gray-500 block">Funcionário Atual</span>
                <span className="font-medium text-gray-900">{quickReplaceData.old_employee_name}</span>
              </div>
            </div>
          </div>

          <div>
            <label className="label-premium">Substituir por *</label>
            <select
              value={quickReplaceData.new_employee_id}
              onChange={(e) => setQuickReplaceData({ ...quickReplaceData, new_employee_id: e.target.value })}
              className="select-premium"
              required
            >
              <option value="">Selecionar novo funcionário...</option>
              {employees
                .filter(e => e.id !== parseInt(quickReplaceData.old_employee_id))
                .map(emp => (
          ...
          <div className="bg-amber-50 p-3 rounded-xl text-xs text-amber-700 flex items-start gap-2">
            ...
            <span>Esta ação substituirá <strong>{quickReplaceData.old_employee_name}</strong> pelo novo funcionário selecionado <strong>apenas neste agendamento</strong>.</span>
          </div>

          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <button type="button" onClick={() => setShowQuickReplaceModal(false)} className="btn-secondary">Cancelar</button>
            <button type="submit" disabled={saving} className="bg-warning text-white px-6 py-2.5 rounded-xl font-medium hover:bg-amber-600 transition-all">
              {saving ? 'Substituindo...' : 'Substituir Funcionário'}
            </button>
          </div>
        </form>
      </Modal>
      ...
      <Modal isOpen={showStatusModal} onClose={() => setShowStatusModal(false)} title="Alterar Status" size="sm">
        <form onSubmit={handleStatusChange} className="space-y-4">
          <div>
            ...
            <select value={statusData.status} onChange={(e) => setStatusData({ ...statusData, status: e.target.value })} className="select-premium" required>
              <option value="">Selecionar...</option>
              {STATUS_OPTIONS.map(s => (<option key={s.value} value={s.value}>{s.label}</option>))}
            </select>
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            <button type="button" onClick={() => setShowStatusModal(false)} className="btn-secondary">Cancelar</button>
      ...
      <div className="card-premium p-4">
        <div className="flex flex-wrap gap-4">
          {[
            { color: 'bg-success', label: 'Confirmado' },
            { color: 'bg-info', label: 'Em andamento' },
            { color: 'bg-warning', label: 'Pendente' },
            { color: 'bg-danger', label: 'Cancelado' },
          ].map((item, idx) => (
