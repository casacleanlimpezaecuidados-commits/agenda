...
module.exports = scheduleController;const { Schedule, Client, Employee, History, Confirmation } = require('../config/database');
...
const scheduleController = {
  ...
  async create(req, res) {
    try {
      ...
      const duplicate = await Schedule.findOne({ client_id, date, start_time, address: address || '', status: { $in: ['pendente', 'confirmado', 'em_andamento'] } });

      if (duplicate) {
        return res.status(409).json({ error: 'Já existe um agendamento para este cliente na mesma data, horário e endereço' });
      }

      const schedule = await Schedule.create({ client_id, employee_ids, date, start_time, end_time, service, address: address || '', notes: notes || '', status: 'pendente', created_by: req.user.id });

      const client = await Client.findById(client_id);
      await History.create({ schedule_id: schedule._id, user_id: req.user.id, action: 'criou_agendamento', new_value: `${date} - ${start_time} - ${client?.name || 'N/A'}`, timestamp: new Date() });
  ...
  async createRecurring(req, res) {
    try {
      ...
      const client = await Client.findById(client_id);
      await History.create({ user_id: req.user.id, action: 'criou_recorrencia', new_value: `${client?.name || 'N/A'} - ${frequency} - ${createdSchedules.length} agendamentos`, timestamp: new Date() });
  ...
  async updateStatus(req, res) {
    try {
      ...
      await History.create({ schedule_id: schedule._id, user_id: req.user.id, action: 'alterou_status', old_value: oldStatus, new_value: status, timestamp: new Date() });

      if (['concluido', 'concluido_ressalva'].includes(status)) {
        await Confirmation.create({ schedule_id: schedule._id, employee_id: req.user.id, status: status, confirmed_at: new Date() });
      }
  ...
  async replaceEmployee(req, res) {
    try {
      const { old_employee_id, new_employee_id, start_date, end_date } = req.body;
      if (!old_employee_id || !new_employee_id) return res.status(400).json({ error: 'IDs obrigatórios' });
      ...
      for (const schedule of schedules) {
        schedule.employee_ids = schedule.employee_ids.map(id => id.toString() === old_employee_id.toString() ? new_employee_id : id);
        await schedule.save();
        updatedCount++;
      }

      await History.create({ user_id: req.user.id, action: 'substituiu_funcionario', old_value: old_employee_id, new_value: new_employee_id, timestamp: new Date() });
  ...
  async getReport(req, res) {
    try {
      const { client_id, month, year } = req.query;
      if (!client_id || !month || !year) return res.status(400).json({ error: 'client_id, month e year são obrigatórios' });
      ...
      const startDate = `${year}-${String(month).padStart(2, '0')}-01`;
      const endDate = `${year}-${String(month).padStart(2, '0')}-31`;
      const schedules = await Schedule.find({ client_id, date: { $gte: startDate, $lte: endDate } }).sort({ date: 1 });

      const weekdays = ['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'];
      const statusLabels = { pendente: 'Pendente', confirmado: 'Confirmado', em_andamento: 'Em Andamento', concluido: 'Concluído', concluido_ressalva: 'Concluído c/ Ressalva', cancelado_cliente: 'Cancelado pelo Cliente', funcionario_faltou: 'Funcionário Faltou' };

      const summary = { total: schedules.length, concluido: schedules.filter(s => s.status === 'concluido').length, concluido_ressalva: schedules.filter(s => s.status === 'concluido_ressalva').length, cancelado_cliente: schedules.filter(s => s.status === 'cancelado_cliente').length, funcionario_faltou: schedules.filter(s => s.status === 'funcionario_faltou').length, outros: schedules.filter(s => ['pendente', 'confirmado', 'em_andamento'].includes(s.status)).length };

      const schedulesDetailed = await Promise.all(schedules.map(async (s) => {
        const employees = await Employee.find({ _id: { $in: s.employee_ids } });
        const scheduleDate = new Date(s.date + 'T00:00:00');
        return { date: s.date, weekday: weekdays[scheduleDate.getDay()] || '', start_time: s.start_time, end_time: s.end_time, service: s.service, address: s.address, employees: employees.map(e => e.name), status: s.status, status_label: statusLabels[s.status] || s.status };
      }));

      const csvHeader = 'Data,Dia,Horário,Serviço,Endereço,Funcionários,Status';
      const csvRows = schedulesDetailed.map(s => { const formattedDate = s.date.split('-').reverse().join('/'); return `${formattedDate},${s.weekday},${s.start_time}-${s.end_time},${s.service},"${s.address}","${s.employees.join(', ')}",${s.status_label}`; });
      const exportCsv = [csvHeader, ...csvRows].join('\n');

      return res.json({ client: { name: client.name, phone: client.phone }, period: `${month}/${year}`, summary, schedules: schedulesDetailed, export_csv: exportCsv });
    } catch (error) {
      console.error('Erro ao gerar relatório:', error);
      return res.status(500).json({ error: error.message });
    }
  ...
  async update(req, res) {
    try {
      ...
      await History.create({ schedule_id: schedule._id, user_id: req.user.id, action: 'atualizou_agendamento', new_value: `${schedule.date} - ${schedule.start_time}`, timestamp: new Date() });
  ...
  async remove(req, res) {
    try {
      ...
      await History.create({ schedule_id: req.params.id, user_id: req.user.id, action: 'removeu_agendamento', old_value: `${schedule.date} - ${schedule.start_time} - ${clientName}`, new_value: 'Agendamento excluído', timestamp: new Date() });
  ...
  async removeByClient(req, res) {
    try {
      const { client_id } = req.params;
      const result = await Schedule.deleteMany({ client_id, status: { $in: ['pendente', 'confirmado', 'em_andamento'] } });

      await History.create({ user_id: req.user.id, action: 'removeu_agendamentos_lote', old_value: `Cliente ID: ${client_id}`, new_value: `${result.deletedCount} agendamentos excluídos`, timestamp: new Date() });

      return res.json({ message: `${result.deletedCount} agendamentos excluídos com sucesso`, deleted_count: result.deletedCount });
    } catch (error) {
      console.error('Erro ao excluir em lote:', error);
      return res.status(500).json({ error: error.message });
    }
  ...
  async sendDailyScheduleWhatsApp(req, res) {
    try {
      ...
      const schedules = await Schedule.find({ date }).sort({ start_time: 1 });
      ...
      for (const phone of phones) {
        try {
          await sendDailySchedule(phone, date, enriched);
          sent++;
        } catch (error) {
      ...
      return res.json({
        message: `Agenda enviada para ${sent} número(s)${errors > 0 ? ` (${errors} falha(s))` : ''}`,
        sent,
        errors,
...
function generateRecurringDates(startDate, endDate, frequency, daysOfWeek) {
  ...
  const dayMap = { 'Dom': 0, 'Seg': 1, 'Ter': 2, 'Qua': 3, 'Qui': 4, 'Sex': 5, 'Sab': 6, 'Segunda': 1, 'Terça': 2, 'Quarta': 3, 'Quinta': 4, 'Sexta': 5, 'Sábado': 6, 'Domingo': 0 };
  const targetDays = daysOfWeek.map(d => dayMap[d] !== undefined ? dayMap[d] : parseInt(d));

  if (frequency === 'semanal') {
    let current = new Date(start);
    while (current <= end) {
      if (targetDays.includes(current.getDay())) dates.push(current.toISOString().split('T')[0]);
      current.setDate(current.getDate() + 1);
    }
  } else if (frequency === 'quinzenal') {
    let current = new Date(start);
    ...
    while (current <= end) {
      if (targetDays.includes(current.getDay())) {
        if (lastFoundDate === null) { dates.push(current.toISOString().split('T')[0]); lastFoundDate = new Date(current); }
        else { const diffDays = Math.floor((current - lastFoundDate) / (1000 * 60 * 60 * 24)); if (diffDays >= 14) { dates.push(current.toISOString().split('T')[0]); lastFoundDate = new Date(current); } }
      }
      current.setDate(current.getDate() + 1);
    }
  } else if (frequency === 'mensal') {
    const dayOfMonth = start.getDate();
    let current = new Date(start.getFullYear(), start.getMonth(), dayOfMonth);
    while (current <= end) { if (current >= start && current <= end) dates.push(current.toISOString().split('T')[0]); current.setMonth(current.getMonth() + 1); }
  }
